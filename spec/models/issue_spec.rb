...
RSpec.describe Issue, :type => :model do
  ...
  let!(:author_parent_parent_organization) { Organization.find_or_create_by(name: "author parent parent organisation") }
  ...
  before do
    ...
    user_9.update_attribute(:organization_id, author_parent_parent_organization.id) # let's say user9 is a member of the parent parent organization
    ...
    author_parent_organization.update_attribute(:parent_id, author_parent_parent_organization.id)
  ...
  context "An issue have multiple coauthors" do
    ...
    it "returns an array with the author and the organizations users if the issue has several coauthors-organizations" do
      ...
      expect(issue_7.coauthors.sort).to_not include(user_9)
    end

    it "returns an array with the author and the organizations users if the issue has several coauthors-organizations" do
      issue_7.update_attribute(:coauthors_status, 3) # shared with author organization, parent organization & parent parent organization
      issue_7.update_attribute(:coauthors_organization_id, author_organization.id)
      expect(author_organization.users).to include(user_2)
      expect(author_organization.users).to include(user_7)
      expect(author_organization.users).to_not include(user_8)
      expect(author_organization.users).to_not include(user_9)
      expect(issue_7.coauthors.sort).to eq [user_9, user_8, user_2, user_7].sort
    end
    ...
    describe "visible?" do
      ...
      it "allows a user to see an issue created by a coauthor from a child organization" do
        ...
        expect(issue_7.visible?(user_9)).to be false
      end
      ...
      it "allows a user to see an issue created by a coauthor from a child organization" do
        ...
        expect(issue_7.visible?(user_7)).to be true
        expect(issue_7.visible?(user_8)).to be true
        expect(issue_7.visible?(user_9)).to be true
      end
    end
    ...
    describe "notified_users" do
      ...
      it "notifies all co-authors and organization parent users when status is 2" do
        ...
        expect(notified_users).to_not include user_9 # not co-author
      end

      it "notifies all co-authors and organization parent users when status is 3" do
        issue_7.update_attribute(:coauthors_status, 3) # shared with author organization & parent organization
        user_8.update_attribute(:mail_notification, 'all')
        user_9.update_attribute(:mail_notification, 'all')
        notified_users = issue_7.notified_users
        expect(notified_users).to_not be_nil
        expect(notified_users).to_not include User.anonymous
        expect(notified_users).to include user_2 # author
        expect(notified_users).to include user_7 # co-author
        expect(notified_users).to include user_8 # co-author as member of parent organization
        expect(notified_users).to include user_9 # co-author as member of parent parent organization
      end
    end
